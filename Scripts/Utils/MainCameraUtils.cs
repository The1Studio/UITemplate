namespace TheOneStudio.UITemplate.UITemplate.Utils
{
    ...
    public static class MainCameraUtils
    {
        ...
        {
            GetMainCam.transform.position = new(position.x, position.y, -10);
        }

        public static void SetOrthographicSize(float size)
        {
            GetMainCam.orthographicSize = size;
        }

        public static void ZoomCamera(float targetSize, float duration, Ease easing)
        {
            GetMainCam.DOOrthoSize(targetSize, duration).SetEase(easing).SetAutoKill(true);
        }
        ...
        public static void AdjustOrthographicHorizontalSize(float horizontalSize)
        {
            ...
            SetCameraPosition(new(0, -1 - (float)1920 / Screen.height));
        }
    }
}