namespace TheOne.Tool.Optimization.Audio
{
    ...
    public class AddressableAudioFinderOdin : OdinEditorWindow
    {
        ...
        [TableList]
        [Title("Wrong Compression Audios", TitleAlignment = TitleAlignments.Centered)]
        private HashSet<AudioInfo> wrongCompressionAudioInfoList = new();

        [ShowInInspector]
        [ListDrawerSettings(NumberOfItemsPerPage = 4)]
        [TableList]
        [Title("Compressed Short Audios", TitleAlignment = TitleAlignments.Centered)]
        private HashSet<AudioInfo> shortCompressedAudioInfoList = new();

        [ShowInInspector]
        [ListDrawerSettings(NumberOfItemsPerPage = 4)]
        [TableList]
        [Title("Compressed Long Audios", TitleAlignment = TitleAlignments.Centered)]
        private HashSet<AudioInfo> longCompressedAudioInfoList = new();

        [ShowInInspector] [HideReferenceObjectPicker] [Title("Audio Compress Settings", TitleAlignment = TitleAlignments.Centered)] private AudioCompressSetting audioCompressSetting = new();

        [MenuItem("TheOne/List And Optimize/Audio List")]
        private static void OpenWindow()
        {
            GetWindow<AddressableAudioFinderOdin>().Show();
        }

        [ButtonGroup("Action")]
        [Button(ButtonSizes.Medium)]
        private void FindAllAudioAndImporter()
        {
        ...
        private bool IsWrongCompression(AudioImporter audioImporter, AudioClip audioClip, bool normalized)
        {
            ...
            isWrongCompression = !audioImporter.forceToMono || audioSetting.loadType != (isLongSound ? AudioClipLoadType.CompressedInMemory : AudioClipLoadType.DecompressOnLoad) || audioSetting.preloadAudioData != !isLongSound || audioSetting.compressionFormat != AudioCompressionFormat.Vorbis || Math.Abs(audioSetting.quality - this.audioCompressSetting.quality) > 0.0001f || normalized;

            return isWrongCompression;
        }

        private bool IsLongSound(AudioClip item)
        {
            return item.length >= this.audioCompressSetting.longAudioLength;
        }
        ...
        public void CheckDuplicateAudios()
        {
            ...
            for (var j = i + 1; j < audioClips.Count; j++)
            {
                ...
                if (this.DuplicateAudios.All(x => !x.Contains(audioClips[i])))
                    this.DuplicateAudios.Add(new() { audioClips[i], audioClips[j] });
                else
            ...
            if (!this.DuplicateAudios.Any()) this.ShowNotification(new("No Duplicate Audios"));
        }
        ...
        [BoxGroup("---")] [ShowIf("@this.DuplicateAudios.Count > 0")] public List<HashSet<AudioClip>> DuplicateAudios = new();
    ...
    {
        [HideLabel] [PreviewField(90, ObjectFieldAlignment.Left)] [HorizontalGroup("row2", 90)] [VerticalGroup("row2/left")] [ShowInInspector] public AudioClip AudioPreview { get; set; }

        [VerticalGroup("row2/right")] [ShowInInspector] public AudioClip Audio { get; set; }

        [VerticalGroup("row2/right")] [ShowInInspector] public AudioImporter Importer { get; set; }

        [VerticalGroup("row2/right")] [ShowInInspector] public bool ForceToMono { get; set; }

        [VerticalGroup("row2/right")] [ShowInInspector] public bool Normalize { get; set; }

        [VerticalGroup("row2/right")] [ShowInInspector] public bool LoadInBackground { get; set; }

        [VerticalGroup("row2/right")] [ShowInInspector] public AudioClipLoadType LoadType { get; set; }

        [VerticalGroup("row2/right")] [ShowInInspector] public bool PreloadAudioData { get; set; }

        [VerticalGroup("row2/right")] [ShowInInspector] public AudioCompressionFormat CompressionFormat { get; set; }

        [VerticalGroup("row2/right")] [ShowInInspector] [PropertyRange(0, 1)] public float Quality { get; set; }
    }
}