...
namespace BuildReportTool.Window.Screen
{
    ...
    {
        public override string Name => Labels.OPTIONS_CATEGORY_LABEL;

        private string[] _saveTypeLabels;

        /// <summary>
        /// 0: always use configured file filters <br/>
        /// 1: use file filters embedded in opened build report, if available
        /// </summary>
        private static readonly string[] FileFilterToUseType =
        ...
        /// </summary>
        private static readonly string[] ShowThumbnailOnHoverTypeLabels =
        ...
        /// </summary>
        private static readonly string[] AssetPingTypeLabels =
        ...
        /// 2: minimal
        /// </summary>
        private static readonly string[] AssetUsageLabelTypeLabels =
        {
            "Verbose\n(use words only)",
        ...
        private static readonly string[] SearchTypeLabels =
        {
            "Basic",
            "Regex",
            "Fuzzy Search",
        ...
        private string OPEN_IN_FILE_BROWSER_OS_SPECIFIC_LABEL
        {
            get
            {
                if (Util.IsInWinOS) return Labels.OPEN_IN_FILE_BROWSER_WIN_LABEL;
                if (Util.IsInMacOS) return Labels.OPEN_IN_FILE_BROWSER_MAC_LABEL;

                return Labels.OPEN_IN_FILE_BROWSER_DEFAULT_LABEL;
            }
        }

        private string SAVE_PATH_TYPE_PERSONAL_OS_SPECIFIC_LABEL
        {
            get
            {
                if (Util.IsInWinOS) return Labels.SAVE_PATH_TYPE_PERSONAL_WIN_LABEL;
                if (Util.IsInMacOS) return Labels.SAVE_PATH_TYPE_PERSONAL_MAC_LABEL;

                return Labels.SAVE_PATH_TYPE_PERSONAL_DEFAULT_LABEL;
            }
        }

        private static readonly string[] CalculationTypeLabels =
        {
        ...
        private int _selectedCalculationLevelIdx;

        private string CalculationLevelDescription
        {
            get
            {
                switch (this._selectedCalculationLevelIdx)
                {
                    case 0: return Labels.CALCULATION_LEVEL_FULL_DESC;
                    case 1: return Labels.CALCULATION_LEVEL_NO_PREFAB_DESC;
                    case 2: return Labels.CALCULATION_LEVEL_NO_UNUSED_DESC;
                    case 3: return Labels.CALCULATION_LEVEL_MINIMAL_DESC;
                }
        ...
        private int GetCalculationLevelGuiIdxFromOptions()
        {
            if (BuildReportTool.Options.IsCurrentCalculationLevelAtFull) return 0;

            if (BuildReportTool.Options.IsCurrentCalculationLevelAtNoUnusedPrefabs) return 1;

            if (BuildReportTool.Options.IsCurrentCalculationLevelAtNoUnusedAssets) return 2;

            if (BuildReportTool.Options.IsCurrentCalculationLevelAtOverviewOnly) return 3;

            return 0;
        }

        private void SetCalculationLevelFromGuiIdx(int selectedIdx)
        {
            switch (selectedIdx)
            {
                case 0:
        ...
        {
            if (this._saveTypeLabels == null) this._saveTypeLabels = new[] { this.SAVE_PATH_TYPE_PERSONAL_OS_SPECIFIC_LABEL, Labels.SAVE_PATH_TYPE_PROJECT_LABEL };

            this._selectedCalculationLevelIdx = this.GetCalculationLevelGuiIdxFromOptions();
        }

        private GUIStyle _linkStyle;
        private GUIStyle _textBesideLinkStyle;

        private static readonly GUILayoutOption[] LayoutMinWidth100            = new[] { GUILayout.MinWidth(100) };
        private static readonly GUILayoutOption[] LayoutMinWidth200            = new[] { GUILayout.MinWidth(200) };
        private static readonly GUILayoutOption[] LayoutMinWidth250            = new[] { GUILayout.MinWidth(250) };
        private static readonly GUILayoutOption[] LayoutMaxWidth525            = new[] { GUILayout.MaxWidth(525) };
        private static readonly GUILayoutOption[] LayoutMaxWidth593            = new[] { GUILayout.MaxWidth(593) };
        private static readonly GUILayoutOption[] LayoutMaxWidth848            = new[] { GUILayout.MaxWidth(848) };
        private static readonly GUILayoutOption[] LayoutMaxWidth500MinHeight75 = new[] { GUILayout.MaxWidth(500), GUILayout.MinHeight(75) };

        private static readonly GUILayoutOption[] LayoutWidth300 = new[] { GUILayout.Width(300) };
        private static readonly GUILayoutOption[] LayoutHeight26 = new[] { GUILayout.Height(26) };

        private static readonly GUILayoutOption[] LayoutNoExpandWidth = new[] { GUILayout.ExpandWidth(false) };

        private          string     _hoveredControlTooltipText;
        private readonly GUIContent _tooltipLabel = new();

        private          ReorderableList _ignorePatternList;
        private readonly GUIContent      _basicSearchRadioLabel = new("Basic");
        private readonly GUIContent      _regexSearchRadioLabel = new("Regex");

        private Texture2D _iconValid;
        private Texture2D _iconInvalid;
        ...
        {
            if (Event.current.type == EventType.Repaint) this._hoveredControlTooltipText = null;

            var validityStyle = GUI.skin.FindStyle("IconValidity");
            if (validityStyle != null)
            {
                this._iconValid   = validityStyle.normal.background;
                this._iconInvalid = validityStyle.hover.background;
            }
            ...
            if (boxedLabelStyle == null) boxedLabelStyle = GUI.skin.box;

            var header1Style                       = GUI.skin.FindStyle(Settings.INFO_TITLE_STYLE_NAME);
            if (header1Style == null) header1Style = GUI.skin.label;

            var header2Style                       = GUI.skin.FindStyle(Settings.SUB_TITLE_STYLE_NAME);
            if (header2Style == null) header2Style = GUI.skin.label;

            var prevEnabled = GUI.enabled;

            GUILayout.Space(10); // extra top padding

            this._assetListScrollPos = GUILayout.BeginScrollView(this._assetListScrollPos, BRT_BuildReportWindow.LayoutNone);
            ...
            if (!string.IsNullOrEmpty(BuildReportTool.Options.FoundPathForSavedOptions))
            {
                ...
                if (GUILayout.Button("Reload", BRT_BuildReportWindow.LayoutNone)) BuildReportTool.Options.RefreshOptions();
            ...
            GUILayout.BeginVertical(BRT_BuildReportWindow.LayoutNone);
            var newSelectedCalculationLevelIdx = EditorGUILayout.Popup(this._selectedCalculationLevelIdx,
                CalculationTypeLabels,
                "Popup",
                LayoutWidth300);
            GUILayout.BeginHorizontal(BRT_BuildReportWindow.LayoutNone);
            GUILayout.Space(20);
            GUILayout.Label(this.CalculationLevelDescription, LayoutMaxWidth500MinHeight75);
            GUILayout.EndHorizontal();
            ...
            if (newSelectedCalculationLevelIdx != this._selectedCalculationLevelIdx)
            {
                this._selectedCalculationLevelIdx = newSelectedCalculationLevelIdx;
                this.SetCalculationLevelFromGuiIdx(newSelectedCalculationLevelIdx);
            }
            ...
            {
                this._linkStyle                  = new(GUI.skin.label);
                this._linkStyle.normal.textColor = new(0.266f, 0.533f, 1);
                this._linkStyle.hover.textColor  = new(0.118f, 0.396f, 1);
                this._linkStyle.stretchWidth     = false;
                this._linkStyle.margin.bottom    = 0;
                this._linkStyle.padding.bottom   = 0;
            }

            if (this._textBesideLinkStyle == null)
            {
                this._textBesideLinkStyle                = new(GUI.skin.label);
                this._textBesideLinkStyle.stretchWidth   = false;
                this._textBesideLinkStyle.margin.right   = 0;
                this._textBesideLinkStyle.padding.right  = 0;
                this._textBesideLinkStyle.margin.bottom  = 0;
                this._textBesideLinkStyle.padding.bottom = 0;
            }

            GUILayout.BeginHorizontal(BRT_BuildReportWindow.LayoutNone);
            GUILayout.Label("Note: This option is a workaround for the ", this._textBesideLinkStyle, BRT_BuildReportWindow.LayoutNone);
            if (GUILayout.Button("Unity bug with Issue ID 885258", this._linkStyle, BRT_BuildReportWindow.LayoutNone))
                Application.OpenURL(
            ...
            if (string.IsNullOrEmpty(entriesPerBatchInput)) entriesPerBatchInput = "0";
            ...
            if (this._ignorePatternList == null)
            {
                this._ignorePatternList                    = new(BuildReportTool.Options.IgnorePatternsForUnused, typeof(SavedOptions.IgnorePattern));
                this._ignorePatternList.onAddCallback      = OnAddPattern;
                this._ignorePatternList.drawHeaderCallback = rect => GUI.Label(rect, "Ignore Patterns for Unused Assets");
                this._ignorePatternList.elementHeight      = 25;
                this._ignorePatternList.drawElementCallback =
                    (elementRect, index, isActive, isFocused) =>
                    {
                        ...
                        var basicSearchSize = radioLeftStyle.CalcSize(this._basicSearchRadioLabel);
                        var regexSearchSize = radioRightStyle.CalcSize(this._regexSearchRadioLabel);

                        var spacing = 3;

                        var textFieldRect = new Rect(elementRect);
                        textFieldRect.y      += 4;
                        ...
                        {
                            if (this._iconValid != null && this._iconInvalid != null)
                            {
                                spacing             += 18;
                                textFieldRect.width -= spacing;
                                EditorGUI.DrawTextureTransparent(new(textFieldRect.xMax + 3, elementRect.y + 5, 16, 16),
                                    Util.IsRegexValid(element.Pattern) ? this._iconValid : this._iconInvalid);
                            }
                            else
                            {
                                spacing             += 50;
                                textFieldRect.width -= spacing;
                                GUI.Label(new(textFieldRect.xMax + 3, elementRect.y + 5, 50, 16),
                        ...
                        var basicToggleRect = new Rect(textFieldRect.xMax + spacing, elementRect.y + 2, basicSearchSize.x, basicSearchSize.y);
                        var pressedBasic = GUI.Toggle(basicToggleRect,
                            element.SearchType == SavedOptions.SEARCH_METHOD_BASIC,
                            this._basicSearchRadioLabel,
                            radioLeftStyle);
                        var basicChanged                     = pressedBasic && element.SearchType != SavedOptions.SEARCH_METHOD_BASIC;
                        if (basicChanged) element.SearchType = SavedOptions.SEARCH_METHOD_BASIC;

                        var regexToggleRect = new Rect(textFieldRect.xMax + spacing + basicSearchSize.x, elementRect.y + 2, regexSearchSize.x, regexSearchSize.y);
                        var pressedRegex = GUI.Toggle(regexToggleRect,
                            element.SearchType == SavedOptions.SEARCH_METHOD_REGEX,
                            this._regexSearchRadioLabel,
                            radioRightStyle);
                ...
                this._ignorePatternList.onChangedCallback = OnIgnorePatternChanged;
            }

            GUILayout.BeginVertical(LayoutMaxWidth848);
            this._ignorePatternList.DoLayoutList();
            GUILayout.EndVertical();
            GUILayout.Space(1);
            ...
            BuildReportTool.Options.CollectTextureImportSettings = GUILayout.Toggle(
                ...
                "Collect Texture Import Settings upon creation of a build report",
                BRT_BuildReportWindow.LayoutNone);
            ...
            BuildReportTool.Options.CollectTextureImportSettingsOnUnusedToo = GUILayout.Toggle(
                ...
                "Include Unused Assets in Texture Import Settings collecting",
                BRT_BuildReportWindow.LayoutNone);
            ...
            GUILayout.Label(string.Format("{0}{1}: {2}", Labels.EDITOR_LOG_LABEL, Util.EditorLogPathOverrideMessage, Util.UsedEditorLogPath),
                BRT_BuildReportWindow.LayoutNone);
            if (GUILayout.Button(this.OPEN_IN_FILE_BROWSER_OS_SPECIFIC_LABEL, BRT_BuildReportWindow.LayoutNone) && Util.UsedEditorLogExists) Util.OpenInFileBrowser(Util.UsedEditorLogPath);
            ...
            if (GUILayout.Button(Labels.SET_OVERRIDE_LOG_LABEL, BRT_BuildReportWindow.LayoutNone))
            {
                ...
                if (!string.IsNullOrEmpty(filepath)) BuildReportTool.Options.EditorLogOverridePath = filepath;
            }

            if (GUILayout.Button(Labels.CLEAR_OVERRIDE_LOG_LABEL, BRT_BuildReportWindow.LayoutNone)) BuildReportTool.Options.EditorLogOverridePath = "";
            ...
            if (string.IsNullOrEmpty(numberOfTopUsedInput)) numberOfTopUsedInput = "0";
            ...
            if (string.IsNullOrEmpty(numberOfTopUnusedInput)) numberOfTopUnusedInput = "0";
            ...
            BuildReportTool.Options.ShowTextureColumnTextureType = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnTextureType,
                "Texture Type",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition))
            {
                this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.TextureType);
                requestRepaint                  = true;
            }

            BuildReportTool.Options.ShowTextureColumnIsSRGB = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnIsSRGB,
                "Is sRGB (Color Texture)",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.IsSRGB);

            BuildReportTool.Options.ShowTextureColumnAlphaSource = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnAlphaSource,
                "Alpha Source",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.AlphaSource);

            BuildReportTool.Options.ShowTextureColumnAlphaIsTransparency = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnAlphaIsTransparency,
                "Alpha Is Transparency",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.AlphaIsTransparency);

            BuildReportTool.Options.ShowTextureColumnIgnorePngGamma = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnIgnorePngGamma,
                "Ignore PNG Gamma",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.IgnorePngGamma);

            BuildReportTool.Options.ShowTextureColumnNPotScale = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnNPotScale,
                "Non-Power of 2 Scale",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.NPotScale);

            BuildReportTool.Options.ShowTextureColumnIsReadable = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnIsReadable,
                "Read/Write Enabled",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.IsReadable);
            ...
            BuildReportTool.Options.ShowTextureColumnMipMapGenerated = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnMipMapGenerated,
                "Mip Map Generated",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.MipMapGenerated);

            BuildReportTool.Options.ShowTextureColumnMipMapFilter = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnMipMapFilter,
                "Mip Map Filter",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.MipMapFilter);

            BuildReportTool.Options.ShowTextureColumnStreamingMipMaps = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnStreamingMipMaps,
                "Streaming Mip Maps",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.StreamingMipMaps);

            BuildReportTool.Options.ShowTextureColumnBorderMipMaps = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnBorderMipMaps,
                "Border Mip Maps",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.BorderMipMaps);

            BuildReportTool.Options.ShowTextureColumnPreserveCoverageMipMaps = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnPreserveCoverageMipMaps,
                "Preserve Coverage Mip Maps",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.PreserveCoverageMipMaps);

            BuildReportTool.Options.ShowTextureColumnFadeOutMipMaps = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnFadeOutMipMaps,
                "Fade Mip Maps",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.FadeOutMipMaps);
            ...
            BuildReportTool.Options.ShowTextureColumnSpriteImportMode = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnSpriteImportMode,
                "Sprite Mode",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.SpriteImportMode);

            BuildReportTool.Options.ShowTextureColumnSpritePackingTag = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnSpritePackingTag,
                "Sprite Packing Tag",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.SpritePackingTag);

            BuildReportTool.Options.ShowTextureColumnSpritePixelsPerUnit = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnSpritePixelsPerUnit,
                "Sprite Pixels Per Unit",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.SpritePixelsPerUnit);

            BuildReportTool.Options.ShowTextureColumnQualifiesForSpritePacking = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnQualifiesForSpritePacking,
                "Qualifies for Sprite Packing",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.QualifiesForSpritePacking);

            BuildReportTool.Options.ShowTextureColumnWrapMode = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnWrapMode,
                "Wrap Mode",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.WrapMode);

            BuildReportTool.Options.ShowTextureColumnWrapModeU = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnWrapModeU,
                "Wrap Mode U",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.WrapModeU);

            BuildReportTool.Options.ShowTextureColumnWrapModeV = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnWrapModeV,
                "Wrap Mode V",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.WrapModeV);

            BuildReportTool.Options.ShowTextureColumnWrapModeW = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnWrapModeW,
                "Wrap Mode W",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.WrapModeW);

            BuildReportTool.Options.ShowTextureColumnFilterMode = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnFilterMode,
                "Filter Mode",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.FilterMode);

            BuildReportTool.Options.ShowTextureColumnAnisoLevel = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnAnisoLevel,
                "Anisotropic Filtering Level",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.AnisoLevel);
            ...
            BuildReportTool.Options.ShowTextureColumnMaxTextureSize = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnMaxTextureSize,
                "Max Texture Size",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.MaxTextureSize);

            BuildReportTool.Options.ShowTextureColumnResizeAlgorithm = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnResizeAlgorithm,
                "Resize Algorithm",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.TextureResizeAlgorithm);

            BuildReportTool.Options.ShowTextureColumnTextureFormat = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnTextureFormat,
                "Texture Format",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.TextureFormat);

            BuildReportTool.Options.ShowTextureColumnCompressionType = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnCompressionType,
                "Compression Type",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.CompressionType);

            BuildReportTool.Options.ShowTextureColumnCompressionIsCrunched = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnCompressionIsCrunched,
                "Compression Crunched",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.CompressionIsCrunched);

            BuildReportTool.Options.ShowTextureColumnCompressionQuality = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnCompressionQuality,
                "Compression Quality",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.CompressionQuality);

            BuildReportTool.Options.ShowTextureColumnImportedWidthAndHeight = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnImportedWidthAndHeight,
                "Imported Width & Height",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.ImportedWidthAndHeight);

            BuildReportTool.Options.ShowTextureColumnRealWidthAndHeight = GUILayout.Toggle(
                BuildReportTool.Options.ShowTextureColumnRealWidthAndHeight,
                "Source Width & Height",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = TextureData.GetTooltipTextFromId(TextureData.DataId.RealWidthAndHeight);
            ...
            BuildReportTool.Options.ShowMeshColumnMeshFilterCount = GUILayout.Toggle(
                BuildReportTool.Options.ShowMeshColumnMeshFilterCount,
                "Non-Skinned Mesh Count",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = MeshData.GetTooltipTextFromId(MeshData.DataId.MeshFilterCount);

            BuildReportTool.Options.ShowMeshColumnSkinnedMeshRendererCount = GUILayout.Toggle(
                BuildReportTool.Options.ShowMeshColumnSkinnedMeshRendererCount,
                "Skinned Mesh Count",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = MeshData.GetTooltipTextFromId(MeshData.DataId.SkinnedMeshRendererCount);
            ...
            BuildReportTool.Options.ShowMeshColumnSubMeshCount = GUILayout.Toggle(
                BuildReportTool.Options.ShowMeshColumnSubMeshCount,
                "Sub-mesh Count",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = MeshData.GetTooltipTextFromId(MeshData.DataId.SubMeshCount);

            BuildReportTool.Options.ShowMeshColumnVertexCount = GUILayout.Toggle(
                BuildReportTool.Options.ShowMeshColumnVertexCount,
                "Vertex Count",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = MeshData.GetTooltipTextFromId(MeshData.DataId.VertexCount);

            BuildReportTool.Options.ShowMeshColumnTriangleCount = GUILayout.Toggle(
                BuildReportTool.Options.ShowMeshColumnTriangleCount,
                "Face Count",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = MeshData.GetTooltipTextFromId(MeshData.DataId.TriangleCount);
            ...
            BuildReportTool.Options.ShowMeshColumnAnimationType = GUILayout.Toggle(
                BuildReportTool.Options.ShowMeshColumnAnimationType,
                "Animation Type",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = MeshData.GetTooltipTextFromId(MeshData.DataId.AnimationType);

            BuildReportTool.Options.ShowMeshColumnAnimationClipCount = GUILayout.Toggle(
                BuildReportTool.Options.ShowMeshColumnAnimationClipCount,
                "Animation Clip Count",
                BRT_BuildReportWindow.LayoutNone);
            if (Event.current.type == EventType.Repaint && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)) this._hoveredControlTooltipText = MeshData.GetTooltipTextFromId(MeshData.DataId.AnimationClipCount);
            ...
            var pageInput = GUILayout.TextField(BuildReportTool.Options.AssetListPaginationLength.ToString(), LayoutMinWidth100);
            pageInput = Regex.Replace(pageInput, @"[^0-9]", ""); // positive numbers only, no fractions
            if (string.IsNullOrEmpty(pageInput)) pageInput = "0";
            ...
            if (string.IsNullOrEmpty(logMessagesInput)) logMessagesInput = "0";
            ...
            BuildReportTool.Options.SearchTypeInt = GUILayout.SelectionGrid(
                BuildReportTool.Options.SearchTypeInt,
                SearchTypeLabels,
                3,
                BRT_BuildReportWindow.LayoutNone);
            GUILayout.FlexibleSpace();
            ...
            BuildReportTool.Options.SearchFilenameOnly = GUILayout.Toggle(
                ...
                "Search through filenames only (ignore path when searching)",
                BRT_BuildReportWindow.LayoutNone);
            ...
            if (GUILayout.Button(this.OPEN_IN_FILE_BROWSER_OS_SPECIFIC_LABEL, BRT_BuildReportWindow.LayoutNone)) Util.OpenInFileBrowser(FiltersUsed.GetProperFileFilterGroupToUseFilePath());
            ...
            BuildReportTool.Options.AssetUsageLabelType = GUILayout.SelectionGrid(
                BuildReportTool.Options.AssetUsageLabelType,
                AssetUsageLabelTypeLabels,
                1,
                BRT_BuildReportWindow.LayoutNone);
            GUILayout.FlexibleSpace();
            ...
            BuildReportTool.Options.ShowThumbnailOnHoverType = GUILayout.SelectionGrid(
                BuildReportTool.Options.ShowThumbnailOnHoverType,
                ShowThumbnailOnHoverTypeLabels,
                ShowThumbnailOnHoverTypeLabels.Length,
                LayoutHeight26);
            GUILayout.FlexibleSpace();
            GUILayout.EndHorizontal();
            ...
            var tooltipThumbnailWidthInput =
            ...
            if (string.IsNullOrEmpty(tooltipThumbnailWidthInput)) tooltipThumbnailWidthInput = "0";
            ...
            var tooltipThumbnailHeightInput =
            ...
            if (string.IsNullOrEmpty(tooltipThumbnailHeightInput)) tooltipThumbnailHeightInput = "0";
            ...
            var tooltipThumbnailZoomedInWidthInput =
            ...
            if (string.IsNullOrEmpty(tooltipThumbnailZoomedInWidthInput)) tooltipThumbnailZoomedInWidthInput = "0";
            ...
            GUILayout.Label("Height:", BRT_BuildReportWindow.LayoutNone);
            var tooltipThumbnailZoomedInHeightInput =
            ...
            if (string.IsNullOrEmpty(tooltipThumbnailZoomedInHeightInput)) tooltipThumbnailZoomedInHeightInput = "0";
            ...
            if (GUILayout.Button(this.OPEN_IN_FILE_BROWSER_OS_SPECIFIC_LABEL, BRT_BuildReportWindow.LayoutNone)) Util.OpenInFileBrowser(BuildReportTool.Options.BuildReportSavePath);
            ...
            if (this._saveTypeLabels == null)
                this._saveTypeLabels = new[]
                {
                    this.SAVE_PATH_TYPE_PERSONAL_OS_SPECIFIC_LABEL, Labels.SAVE_PATH_TYPE_PROJECT_LABEL,
                };

            BuildReportTool.Options.SaveType = GUILayout.SelectionGrid(BuildReportTool.Options.SaveType,
                this._saveTypeLabels,
                this._saveTypeLabels.Length,
                BRT_BuildReportWindow.LayoutNone);
            GUILayout.FlexibleSpace();
            GUILayout.EndHorizontal();
            ...
            if (Event.current.type == EventType.Repaint && !string.IsNullOrEmpty(this._hoveredControlTooltipText))
            {
                this._tooltipLabel.text = this._hoveredControlTooltipText;
                var tooltipTextStyle                           = GUI.skin.FindStyle("TooltipText");
                if (tooltipTextStyle == null) tooltipTextStyle = GUI.skin.label;

                const int MAX_TOOLTIP_WIDTH = 240;
                var       tooltipSize       = tooltipTextStyle.CalcSize(this._tooltipLabel);
                if (tooltipSize.x > MAX_TOOLTIP_WIDTH)
                {
                    tooltipSize.x = MAX_TOOLTIP_WIDTH;
                    tooltipSize.y = tooltipTextStyle.CalcHeight(this._tooltipLabel, tooltipSize.x);
                }

                var tooltipRect = BRT_BuildReportWindow.DrawTooltip(position, tooltipSize.x, tooltipSize.y, 5);
                GUI.Label(tooltipRect, this._tooltipLabel, tooltipTextStyle);
            }
        }

        private static void OnAddPattern(ReorderableList list)
        {
        ...
        private static void OnIgnorePatternChanged(ReorderableList list)
        {
            BuildReportTool.Options.SaveOptions();
        }
    }
