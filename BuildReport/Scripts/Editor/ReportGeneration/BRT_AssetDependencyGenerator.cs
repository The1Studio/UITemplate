...
namespace BuildReportTool
{
    ...
    public static class AssetDependencyGenerator
    {
        ...
        private static string[] GetDependencies(string pathName, bool recursive = false)
        {
        ...
        public static void CreateFromIncludedScenes(
            AssetDependencies data,
            bool              debugLog                 = false,
        ...
        public static void CreateForUsedAssetsOnly(
            AssetDependencies data,
            BuildInfo         buildInfo,
            bool              debugLog = false
        )
        ...
        public static void CreateForAllAssets(
            AssetDependencies data,
            BuildInfo         buildInfo,
            bool              debugLog = false
        )
        {
            ...
            if (buildInfo.UsedAssets != null && buildInfo.UsedAssets.All != null)
            {
                ...
                for (int n = 0, len = allUsedAssets.Length; n < len; ++n)
                {
                    if (string.IsNullOrEmpty(allUsedAssets[n].Name)) continue;
            ...
            if (buildInfo.UnusedAssets != null && buildInfo.UnusedAssets.All != null)
            {
                ...
                {
                    if (string.IsNullOrEmpty(allUnusedAssets[n].Name)) continue;
        ...
        public static bool IsFileTypeBeforeAnother(
            List<AssetUserFlattened> list,
            int                      idx,
            string                   fileTypeInPrevious,
            string                   fileTypeInIdx,
            out int                  idxOfPrevious
        )
        {
            if (idx <= 0)
            {
        ...
        private static bool IsInstanceOfFbxUsingMaterial(
            string                              fbxAssetPath,
            string                              assetUsingFbxAssetPath,
            string                              materialAssetPath,
            Dictionary<string, DependencyEntry> assetDependencies,
            bool                                debugLog = false
        )
        {
            if (!assetDependencies.ContainsKey(assetUsingFbxAssetPath)) return false;
        ...
        private static bool AreInstancesOfFbxUsingMaterial(
            string                              fbxAssetPath,
            string                              materialAssetPath,
            Dictionary<string, DependencyEntry> assetDependencies,
            bool                                debugLog = false
        ...
        {
            if (!assetDependencies.ContainsKey(fbxAssetPath)) return false;

            var assetsThatUseTheFbx = assetDependencies[fbxAssetPath].Users;

            for (int n = 0, len = assetsThatUseTheFbx.Count; n < len; ++n)
            {
                if (debugLog) Debug.LogFormat("assetsThatUseTheFbx {0}: {1}", n, assetsThatUseTheFbx[n]);

                if (IsInstanceOfFbxUsingMaterial(fbxAssetPath,
                    assetsThatUseTheFbx[n],
                    materialAssetPath,
                    assetDependencies,
                    debugLog))
                    return true;
        ...
        /// <returns></returns>
        private static string GetAssetUsageParent(List<AssetUserFlattened> list, int idx)
        {
        ...
        /// <returns></returns>
        private static bool Contains(List<AssetUserFlattened> list, AssetUserFlattened check, string assetParentOfCheck)
        {
            for (var n = list.Count - 1; n >= 0; --n)
            {
                ...
                if (list[n].AssetPath.Equals(check.AssetPath, StringComparison.OrdinalIgnoreCase) && list[n].IndentLevel == check.IndentLevel && GetAssetUsageParent(list, n) == assetParentOfCheck) return true;
            }
        ...
        )
        {
            ...
            while (openSet.Count > 0)
            {
                ...
                if (string.IsNullOrEmpty(newAssetToInspect)) continue;

                closedSet.Add(newAssetToInspect);

                var foundDependencies = GetDependencies(newAssetToInspect, recursiveGetDependencies);
                ...
                {
                    if (string.IsNullOrEmpty(foundDependencies[n])) continue;
                    ...
                    if (newAssetToInspect.IsMaterialFile() && foundDependencies[n].IsTextureFile())
                    {
                        ...
                        for (var pIdx = 0; pIdx < shaderPropertyCount; ++pIdx)
                        {
                    ...
                    if (!assetsUsed.Uses.Contains(foundDependencies[n])) assetsUsed.Uses.Add(foundDependencies[n]);
                    ...
                    if (!assetsUsedByEdit.Users.Contains(newAssetToInspect)) assetsUsedByEdit.Users.Add(newAssetToInspect);
                ...
                if (debugLog) Debug.Log(stringBuilder.ToString());
            }

            // ====================================================================
            // Create the Uses List for each scene in the SceneEntries list

            var openFlattenedSet = new List<AssetUserFlattened>();
            ...
            foreach (var pair in assetDependencies)
            {
                ...
                if (assetUsers.Count == 0) continue;
                ...
                {
                    var newEntry = new AssetUserFlattened(assetUsers[n],
                        1
                ...
                while (openFlattenedSet.Count > 0)
                {
                    ...
                    if (usersFlattened.Count >= 2)
                    {
                        ...
                        if (userAsset.IndentLevel > 1)
                        {
                            ...
                            if (debugLog && pair.Key.Contains("Runtime-Only DLL/TextMeshPro.dll") && userAsset.AssetPath.Contains("AbilityEntry Settings.asset"))
                                Debug.LogFormat(
                                    "=========================================================\nChecking for cyclic dependencies for {0} at index {1}\nLooking for Indent Level {2}",
                                    userAsset.AssetPath,
                                    (usersFlattened.Count - 1).ToString(),
                                    currentIndentLevel);

                            // start just before our own position in the list, then go backwards
                            for (var traceN = usersFlattened.Count - 2; traceN >= 0; --traceN)
                            {
                                if (usersFlattened[traceN].IndentLevel > currentIndentLevel)
                                {
                                    ...
                                    if (debugLog && pair.Key.Contains("Runtime-Only DLL/TextMeshPro.dll") && userAsset.AssetPath.Contains("AbilityEntry Settings.asset"))
                                        Debug.LogFormat("Skipped element {0} since its indent level is: {1} (looking for {2})",
                                            traceN,
                                            usersFlattened[traceN].IndentLevel.ToString(),
                                            currentIndentLevel);
                                ...
                                if (debugLog && pair.Key.Contains("Runtime-Only DLL/TextMeshPro.dll") && userAsset.AssetPath.Contains("AbilityEntry Settings.asset"))
                                    Debug.LogFormat("Now at element {0}: {1} at indent level {2}. Checking if same as {3}",
                                        traceN,
                                        usersFlattened[traceN].AssetPath,
                                        usersFlattened[traceN].IndentLevel.ToString(),
                                        userAsset.AssetPath);
                                ...
                                out idxOfPrevious))
                        {
                            ...
                            if (debugLog)
                                Debug.LogFormat("Detected fbx using a material.\n" + "Material:\n{0}\n" + "Fbx:\n{1}\n" + "Checking the assets that use the fbx if they really also use the material...",
                                    materialAssignedAsDefault,
                                    fbxUsingTheMaterialAsDefault);

                            var isMaterialReallyBeingUsed =
                                AreInstancesOfFbxUsingMaterial(fbxUsingTheMaterialAsDefault,
                                    materialAssignedAsDefault,
                                    assetDependencies,
                                    debugLog);
                        ...
                        if (!noNeedToAddUsersOfUser && userAsset.AssetPath.EndsWith(".cs", StringComparison.OrdinalIgnoreCase))
                        {
                        ...
                        if (noNeedToAddUsersOfUser) continue;
                    }
                    ...
                    if (assetDependencies.ContainsKey(userAsset.AssetPath))
                    {
                        ...
                        {
                            var newEntry = new AssetUserFlattened(usersOfUser[n], userAsset.IndentLevel + 1);

                            if (userAssetIsFbxUsingMaterial)
                            {
                                if (!IsInstanceOfFbxUsingMaterial(userAsset.AssetPath,
                                    usersOfUser[n],
                                    materialAssignedAsDefault,
                                    assetDependencies,
                                    debugLog))
                            ...
                            else if (userAsset.AssetPath.EndsWith(".unity", StringComparison.OrdinalIgnoreCase))
                            {
                                if (usersOfUser[n].EndsWith("LightingData.asset", StringComparison.OrdinalIgnoreCase))
                                {
                                    ...
                                    // The LightingData.asset has a cyclic dependency with its corresponding scene.
                                    // We can ignore this, since this is useless information for the `pair.Key` asset.
                                    if (debugLog)
                                        Debug.LogFormat("Ignoring \"LightingData.asset\" of scene: {0}",
                                            userAsset.AssetPath);
