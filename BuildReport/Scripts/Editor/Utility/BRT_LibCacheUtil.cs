...
public static class BRT_LibCacheUtil
{
    ...
    public static long GetImportedFileSize(string assetPath)
    {
        ...
        if (BuildReportTool.Util.IsFileStreamingAsset(assetPath)) return -1;

        // files like Thumbs.db or .DS_Store files do not get imported
        if (BuildReportTool.Util.IsUselessFile(assetPath)) return -1;

        // Unix-style hidden files do not get imported
        if (BuildReportTool.Util.IsFileAUnixHiddenFile(assetPath)) return -1;
        ...
        if (!string.IsNullOrEmpty(assetPath))
        {
            ...
            else
            {
                ...
                if (File.Exists(assetImportedPath)) result = BuildReportTool.Util.GetFileSizeInBytes(assetImportedPath);
            }
        }
