namespace TheOne.Tool.Optimization.Texture
{
    ...
    public class TextureFinderOdin : OdinEditorWindow
    {
        ...
        [ShowInInspector] [TableList(ShowPaging = true)] [Title("All Textures", TitleAlignment = TitleAlignments.Centered)] private List<TextureInfo> allTextures = new();

        [ShowInInspector] [TableList(ShowPaging = true)] [Title("Mipmap", TitleAlignment = TitleAlignments.Centered)] private List<TextureInfo> generatedMipMap = new();

        [ShowInInspector] [TableList(ShowPaging = true)] [Title("Compressed and Not Crunch Textures", TitleAlignment = TitleAlignments.Centered)] private List<TextureInfo> compressedAndNotCrunchTexture = new();

        [ShowInInspector] [TableList(ShowPaging = true)] [Title("3D Model Texture", TitleAlignment = TitleAlignments.Centered)] private List<TextureInfo> modelTextures = new();

        [ShowInInspector] [TableList(ShowPaging = true)] [Title("All Texture (Not In Models)", TitleAlignment = TitleAlignments.Centered)] private List<TextureInfo> allTexturesNotInModels = new();

        [ShowInInspector] [TableList(ShowPaging = true)] [Title("Not In Atlas", TitleAlignment = TitleAlignments.Centered)] private List<TextureInfo> notInAtlasTexture = new();

        [ShowInInspector] [TableList(ShowPaging = true)] [Title("Duplicated Atlas Textures", TitleAlignment = TitleAlignments.Centered)] private List<TextureInfo> duplicatedAtlasTexture = new();

        [ShowInInspector] [TableList(ShowPaging = true)] [Title("Dont Use Atlas Textures", TitleAlignment = TitleAlignments.Centered)] private List<TextureInfo> dontUseAtlasTexture = new();

        [ShowInInspector] [TableList(ShowPaging = true)] [Title("Not Compressed and Not In Atlas Textures", TitleAlignment = TitleAlignments.Centered)] private List<TextureInfo> notCompressedAndNotInAtlasTexture = new();
        ...
        {
            this.FindTexturesInAssets();
        }

        [EnumToggleButtons] [ShowInInspector] [OnValueChanged(nameof(FindTexturesInAssets))] [BoxGroup("Setting")] private SortingBy sortingBy = SortingBy.TotalPixel;

        [EnumToggleButtons] [ShowInInspector] [OnValueChanged(nameof(FindTexturesInAssets))] [BoxGroup("Setting")] private SortingOrder sortingOrder = SortingOrder.Descending;
        ...
        {
            GetWindow<TextureFinderOdin>().Show();
        }
        ...
        private void FindTexturesInAssets()
        {
            ...
            foreach (var meshRenderer in meshRenderers) modelTextureSet.AddRange(AssetSearcher.GetAllDependencies<Texture>(meshRenderer));

            foreach (var skinnedMeshRenderer in skinnedMeshRenderers) modelTextureSet.AddRange(AssetSearcher.GetAllDependencies<Texture>(skinnedMeshRenderer));
            ...
            foreach (var textureInfo in textureInfos)
            {
                ...
                if (textureInfo.GenerateMipMapEnabled) this.generatedMipMap.Add(textureInfo);

                if (textureInfo.TextureImporter.textureCompression != TextureImporterCompression.Uncompressed && !textureInfo.TextureImporter.crunchedCompression) this.compressedAndNotCrunchTexture.Add(textureInfo);
                ...
                if (!atlasTextureSet.Contains(textureInfo.Texture) && textureInfo.TextureImporter.textureType == TextureImporterType.Sprite) this.notInAtlasTexture.Add(textureInfo);

                if (duplicatedAtlasTextureSet.Contains(textureInfo.Texture)) this.duplicatedAtlasTexture.Add(textureInfo);

                if (textureInfo.CompressionType == TextureImporterCompression.Uncompressed && !atlasTextureSet.Contains(textureInfo.Texture)) this.notCompressedAndNotInAtlasTexture.Add(textureInfo);
            }
        }
            ...
            if (importer != null)
            {
                ...
                if (texture.width > maxSize || texture.height > maxSize)
                {
                    if (texture.width > texture.height)
                        return new(maxSize, maxSize / aspectRatio);
                    else
                        return new(maxSize * aspectRatio, maxSize);
                }
            }
    ...
    {
        [HideLabel] [PreviewField(90, ObjectFieldAlignment.Left)] [HorizontalGroup("group", 90)] [VerticalGroup("group/left")] [ShowInInspector] public Texture Texture { get; set; }

        [VerticalGroup("group/right")] [ShowInInspector] public string Name => this.Texture.name;

        [VerticalGroup("group/right")] [ShowInInspector] [LabelText("File Size (Byte)")] public long FileSize { get; set; }

        [VerticalGroup("group/right")] [ShowInInspector] [LabelText("TinyPNG Compressed File Size (Byte)")] public long TinyPNGCompressedFileSize { get; set; }

        [VerticalGroup("group/right")] [ShowInInspector] public Vector2 TextureSize { get; set; }

        public TextureImporter TextureImporter { get; set; }

        [VerticalGroup("group/right")] [ShowInInspector] public bool ReadWriteEnabled { get; set; }

        [VerticalGroup("group/right")] [ShowInInspector] public bool GenerateMipMapEnabled { get; set; }

        [VerticalGroup("group/right")] [ShowInInspector] public int MaxTextureSize => this.TextureImporter.maxTextureSize;

        [VerticalGroup("group/right")] [ShowInInspector] public TextureImporterCompression CompressionType => this.TextureImporter.textureCompression;

        [VerticalGroup("group/right")] [ShowInInspector] public bool UseCrunchCompression => this.TextureImporter.crunchedCompression;
