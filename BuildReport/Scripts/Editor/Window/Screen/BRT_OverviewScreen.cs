...
namespace BuildReportTool.Window.Screen
{
    public class Overview : BaseScreen
    {
        ...
        private int _assetUsedEntryHoveredIdx   = -1;
        private int _assetUnusedEntryHoveredIdx = -1;
        ...
        )
        {
            ...
            if (EditorGUIUtility.isProSkin || BRT_BuildReportWindow.FORCE_USE_DARK_SKIN) emphasisColor = "white";

            var largestAssetCategoryLabel = string.Format(
                "<color={0}><size=20><b>{1}</b></size></color> are the largest,\ntaking up <color={0}><size=20><b>{2}%</b></size></color> of the build{3}",
                emphasisColor,
                buildReportToDisplay.BuildSizes[0].Name,
            ...
            this.DrawScenesInBuild(buildReportToDisplay.ScenesInBuild);
            ...
            {
                this._showTopUsed   = numberOfTopUsed > 0 && buildReportToDisplay.UsedAssets.TopLargest != null;
                this._showTopUnused = numberOfTopUnused > 0 && buildReportToDisplay.UnusedAssets.TopLargest != null;
            }

            // 1st column
            GUILayout.BeginVertical();
            if (this._showTopUsed)
            {
                ...
                this.DrawAssetList(buildReportToDisplay.UsedAssets, true, buildReportToDisplay, assetDependencies);
            }
            ...
            if (this._showTopUnused)
            {
                ...
                this.DrawAssetList(buildReportToDisplay.UnusedAssets, false, buildReportToDisplay, assetDependencies);
            }
            ...
            if (Event.current.mousePosition.y >= position.height || Event.current.mousePosition.y <= 0 || Event.current.mousePosition.x <= 0 || Event.current.mousePosition.x >= position.width)
            ...
            var showThumbnailNow = BuildReportTool.Options.ShowTooltipThumbnail && this._shouldShowThumbnailOnHoveredAsset && (this._assetUsedEntryHoveredIdx != -1 || this._assetUnusedEntryHoveredIdx != -1);
            ...
            var shouldShowAssetEndUsersTooltipNow = BuildReportTool.Options.ShowAssetPrimaryUsersInTooltipIfAvailable
                && BRT_BuildReportWindow.ShouldHoveredAssetShowEndUserTooltip(
                    assetDependencies)
                && (this._assetUsedEntryHoveredIdx != -1 || this._assetUnusedEntryHoveredIdx != -1);
        ...
        )
        {
            ...
            var assetsToShow = assetList.TopLargest;
            ...
            if (listIconNormalStyle == null) listIconNormalStyle = GUI.skin.label;

            var listIconAltStyle                           = GUI.skin.FindStyle(Settings.LIST_ICON_ALT_STYLE_NAME);
            if (listIconAltStyle == null) listIconAltStyle = GUI.skin.label;
            ...
            // 1st column: name
            GUILayout.BeginVertical();
            for (var n = 0; n < assetsToShow.Length; ++n)
            {
                var b = assetsToShow[n];

                var styleToUse     = useAlt ? listAltStyle : listNormalStyle;
                var iconStyleToUse = useAlt ? listIconAltStyle : listIconNormalStyle;

                if (styleToUse == null) styleToUse         = GUI.skin.label;
                if (iconStyleToUse == null) iconStyleToUse = GUI.skin.label;

                var icon = AssetDatabase.GetCachedIcon(b.Name);
                ...
                var prettyName = string.Format(" {0}. {1}", (n + 1).ToString(), Util.GetAssetFilename(b.Name));
                if (GUILayout.Button(prettyName, styleToUse, BRT_BuildReportWindow.Layout100To400x30)) Utility.PingAssetInProject(b.Name);
            ...
            {
                if (usedAssets)
                    this._assetUsedEntryHoveredIdx = newEntryHoveredIdx;
                else
                    this._assetUnusedEntryHoveredIdx = newEntryHoveredIdx;

                if (newEntryHoveredIdx != -1)
                {
                    var hoveredAssetPath = newEntryHovered != null ? newEntryHovered.Name : null;

                    // ----------------
                    // update what is considered the hovered asset, for use later on
                    // when the tooltip will be drawn
                    BRT_BuildReportWindow.UpdateHoveredAsset(hoveredAssetPath,
                        newEntryHoveredRect,
                        usedAssets,
                    ...
                    if (BuildReportTool.Options.ShowTooltipThumbnail && (BuildReportTool.Options.ShowThumbnailOnHoverLabelToo || hoveringOverIcon) && BRT_BuildReportWindow.GetAssetPreview(hoveredAssetPath) != null)
        ...
        private void DrawScenesInBuild(BuildInfo.SceneInBuild[] scenesInBuild)
        {
            ...
            GUILayout.BeginVertical(BRT_BuildReportWindow.LayoutExpandWidth);
            var usedIdx = -1;
            for (var n = 0; n < scenesInBuild.Length; ++n)
            {
                if (scenesInBuild[n].Enabled) ++usedIdx;

                var styleToUse     = useAlt ? listAltStyle : listNormalStyle;
                var iconStyleToUse = useAlt ? listIconAltStyle : listIconNormalStyle;

                var icon = AssetDatabase.GetCachedIcon(scenesInBuild[n].Path);
                ...
                else
                {
                    ...
                    GUILayout.Button(icon, iconStyleToUse, BRT_BuildReportWindow.Layout28x30);
                    if (!scenesInBuild[n].Enabled) GUI.contentColor = prevColor;
                }
                ...
                if (GUILayout.Button(prettyName, styleToUse, BRT_BuildReportWindow.Layout100x30)) Utility.PingAssetInProject(scenesInBuild[n].Path);
