...
namespace BuildReportTool
{
    public static class Util
    {
        public static int GetUnityMajorVersion(string unityVersion)
        {
            ...
            {
                if (char.IsDigit(unityVersion[majorIdx]) && unityVersion[majorIdx] != '.') majorVersion += unityVersion[majorIdx];

                if (unityVersion[majorIdx] == '.') break;
            }
        ...
        {
            var majorVersion = GetUnityMajorVersion(unityVersionName);
        ...
        public static BuildPlatform GetBuildPlatformBasedOnUnityBuildTarget(BuildTarget b)
        {
            switch (b)
            {
                ...
                #endif
                case BuildTarget.WebGL: return BuildPlatform.WebGL;
                ...
                case BuildTarget.Android: return BuildPlatform.Android;
                ...
                // 8th gen
                case BuildTarget.XboxOne: return BuildPlatform.XBOXOne;
                case BuildTarget.PS4:     return BuildPlatform.PS4;
                ...
                case BuildTarget.Switch: return BuildPlatform.Switch;
                #endif

                // -----------------------------------
                // Windows builds

                case BuildTarget.StandaloneWindows: return BuildPlatform.Windows32;

                case BuildTarget.StandaloneWindows64: return BuildPlatform.Windows64;

                case BuildTarget.WSAPlayer: return BuildPlatform.WindowsStoreApp;
                ...
                case BuildTarget.StandaloneLinux64: return BuildPlatform.Linux64;
                #endif
                ...
                case BuildTarget.StandaloneOSX: return BuildPlatform.MacOSX64;
                #else
        ...
        public static bool ShouldGetBuildReportNow { get => EditorPrefs.GetBool("BRT_ShouldGetBuildReportNow", false); set => EditorPrefs.SetBool("BRT_ShouldGetBuildReportNow", value); }

        public static bool ShouldSaveGottenBuildReportNow { get => EditorPrefs.GetBool("BRT_ShouldSaveGottenBuildReportNow", false); set => EditorPrefs.SetBool("BRT_ShouldSaveGottenBuildReportNow", value); }

        private const string BUILD_TIME_START    = "BRT_UnityBuildTimeStart";
        private const string BUILD_TIME_DURATION = "BRT_UnityBuildTimeDuration";
        ...
        {
            if (HasBuildTime()) SaveBuildTimeDuration(LoadBuildTime(false));
        }
        ...
        {
            var text        = EditorPrefs.GetString(BUILD_TIME_DURATION);
        ...
        public static void SaveUnityBuildReportToCurrent(UnityEditor.Build.Reporting.BuildReport report)
        {
            ...
            string buildType;
            var    gotBuildType = ReportGenerator.GetBuildTypeFromEditorLog(UsedEditorLogPath);
        ...
        public static void SaveUnityBuildReportToTemp(UnityEditor.Build.Reporting.BuildReport report)
        {
            ...
            br.SetFrom(report);
            br.OnBeforeSave();
            var                  x      = new System.Xml.Serialization.XmlSerializer(typeof(UnityBuildReport));
        ...
        public static void DebugLogBuildReport(UnityBuildReport report)
        {
            ...
            for (var i = 0; i < report.OutputFiles.Length; i++)
            {
                sb.AppendFormat("File {0}: {1} ({2}) {3}\n",
                    (i + 1).ToString(),
                    report.OutputFiles[i].FilePath,
                    report.OutputFiles[i].Role,
            ...
            var totalSceneTime = new TimeSpan(0);
            for (var i = 0; i < report.BuildProcessSteps.Length; i++)
            {
                ...
                sb.Append("\n");
                for (var d = 0; d < report.BuildProcessSteps[i].Depth; ++d) sb.Append("    ");
                ...
                if (logs != null && i < report.BuildProcessSteps.Length - 1)
                    ...
                    {
                        sb.Append("\n");
                        for (var d = 0; d < report.BuildProcessSteps[i].Depth + 1; ++d) sb.Append("    ");
        ...
        public static void DebugLogBuildReport(UnityEditor.Build.Reporting.BuildReport report)
        {
            ...
            {
                foreach (var module in report.strippingInfo.includedModules) sb.AppendFormat("\nIncluded Module: {0}", module);
                Debug.Log(sb.ToString());
                sb.Length = 0;
            }
            ...
            for (var i = 0; i < report.steps.Length; i++)
            {
                ...
                sb.Append("\n");
                for (var d = 0; d < report.steps[i].depth; ++d) sb.Append("    ");
                ...
                if (logs != null && i < report.steps.Length - 1)
                    ...
                    {
                        sb.Append("\n");
                        for (var d = 0; d < report.steps[i].depth + 1; ++d) sb.Append("    ");
        ...
        public static BuildTarget BuildTargetOfLastBuild
        {
            get
            {
                var gotBuildTargetIdx = EditorPrefs.GetInt("BRT_BuildTargetOfLastBuild", 0);
                return (BuildTarget)gotBuildTargetIdx;
            }
            set
            {
                var buildTargetIdx = Convert.ToInt32(value);
        ...
        {
            if (val.EndsWith(suffix, StringComparison.OrdinalIgnoreCase)) return val.Substring(0, val.Length - suffix.Length + offset);

            return val;
        }

        private static string GetLastFolder(string inFolder)
        {
            ...
            var lastSlashIdx = inFolder.LastIndexOf('/');
        ...
        {
            var folderEntries = System.IO.Directory.GetDirectories(folderToStart);

            for (int n = 0, len = folderEntries.Length; n < len; ++n)
            {
                var folderName = GetLastFolder(folderEntries[n]);
                ...
                else
                {
                    ...
                    var recursedFolderName = GetLastFolder(recursed);
                    if (recursedFolderName == desiredFolderName) return recursed;
                }
            }
        ...
        private static string GetPathParentFolder(string path)
        {
        ...
        {
            if (string.IsNullOrEmpty(buildReport.BuildFilePath)) return string.Empty;

            var buildPlatform =
                ReportGenerator.GetBuildPlatformFromString(buildReport.BuildType,
                    buildReport.BuildTargetUsed);

            if (buildPlatform == BuildPlatform.Windows32 || buildPlatform == BuildPlatform.Windows64 || buildPlatform == BuildPlatform.Linux32 || buildPlatform == BuildPlatform.Linux64)
            {
                ...
                return string.Format("File size of {0} and the {1} folder in <b><color=white>{2}</color></b>",
                    exeFile,
                    dataFolder,
                    buildParentFolder);
            }
            ...
            if (buildPlatform == BuildPlatform.LinuxUniversal)
            {
                ...
                return string.Format("File size of {0}, {1}, and the {2} folder in <b><color=white>{3}</color></b>",
                    exe32File,
                    exe64File,
                    dataFolder,
                    buildParentFolder);
        ...
        {
            if (string.IsNullOrEmpty(eclipseProjectPath) || !System.IO.Directory.Exists(eclipseProjectPath)) return 0;
        ...
        {
            if (string.IsNullOrEmpty(eclipseProjectPath) || !System.IO.Directory.Exists(eclipseProjectPath)) return string.Empty;
        ...
        {
            if (System.IO.Directory.Exists(fileOrFolder)) return GetFolderSizeInBytes(fileOrFolder);

            if (System.IO.File.Exists(fileOrFolder)) return GetFileSizeInBytes(fileOrFolder);
        ...
        {
            if (string.IsNullOrEmpty(folderPath) || !System.IO.Directory.Exists(folderPath)) return 0;

            double totalBytesOfFilesInFolder                                                         = 0;
            foreach (var file in DldUtil.TraverseDirectory.Do(folderPath)) totalBytesOfFilesInFolder += GetFileSizeInBytes(file);
        ...
        {
            if (string.IsNullOrEmpty(folderPath) || !System.IO.Directory.Exists(folderPath)) return string.Empty;
        ...
        {
            if (string.IsNullOrEmpty(filename) || !System.IO.File.Exists(filename)) return 0;
        ...
        private const double ONE_TERABYTE = 1099511627776.0;
        private const double ONE_GIGABYTE = 1073741824.0;
        private const double ONE_MEGABYTE = 1048576.0;
        private const double ONE_KILOBYTE = 1024.0;

        private const ulong ONE_TERABYTE_L = 1099511627776;
        private const ulong ONE_GIGABYTE_L = 1073741824;
        private const ulong ONE_MEGABYTE_L = 1048576;
        private const ulong ONE_KILOBYTE_L = 1024;
        ...
        private static string MyFileSizeReadable(double bytes)
        {
        ...
        private static string MyFileSizeReadable(ulong bytes)
        {
        ...
        {
            if (string.IsNullOrEmpty(size) || size == "N/A" || size.Length <= 2) return 0;

            // units is expected to be in the last two letters of the string
            var units = size.Substring(size.Length - 2, 2);

            // therefore, everything except the last two letters is expected to be a number
            var numOnly = size.Substring(0, size.Length - 2);
            ...
            switch (units)
            {
                case "KB": return num * ONE_KILOBYTE;
                case "MB": return num * ONE_MEGABYTE;
                case "GB": return num * ONE_GIGABYTE;
                case "TB": return num * ONE_TERABYTE;
            }
        ...
        private static bool TextFileHasContents(string filepath, string contents)
        {
        ...
        {
            var projectFolder = RemoveSuffix("Assets", Application.dataPath);
        ...
        private static void DeleteFile(string projectFolder, string file)
        {
            //Debug.Log("will delete " + file);

            if (HaveToUseSystemForDelete(file))
            {
                var fileAbsPath = System.IO.Path.Combine(projectFolder, file);
        ...
        {
            var idxOfDot = filename.LastIndexOf(".", StringComparison.Ordinal);

            if (idxOfDot < 0)
                // dot is not found
                return string.Empty;

            var newFile = filename.Remove(idxOfDot);
        ...
        {
            return IsFileInAPath(filepath, "/Editor/") || DoesFileStartIn(filepath, "Editor/");
        }
        ...
        {
            if ((filepath.StartsWith("Built-in") && filepath.EndsWith(":")) || filepath.DoesFileHaveInvalidPathChars()) return filepath;
        ...
        {
            return string.Equals(filepath.GetFileNameOnly(),
                filenameToCheck,
                StringComparison.OrdinalIgnoreCase);
        }
        ...
        {
            return !string.IsNullOrEmpty(me) && (me.StartsWith("Assets/", StringComparison.OrdinalIgnoreCase) || me.StartsWith("Packages/", StringComparison.OrdinalIgnoreCase));
        }
        ...
        {
            return IsFileOfType(file, ".psd") || IsFileOfType(file, ".jpg") || IsFileOfType(file, ".jpeg") || IsFileOfType(file, ".gif") || IsFileOfType(file, ".png") || IsFileOfType(file, ".tiff") || IsFileOfType(file, ".tif") || IsFileOfType(file, ".tga") || IsFileOfType(file, ".bmp") || IsFileOfType(file, ".dds") || IsFileOfType(file, ".exr") || IsFileOfType(file, ".iff") || IsFileOfType(file, ".pict");
        }
        ...
        {
            return IsFileOfType(file, ".fbx") || IsFileOfType(file, ".dae") || IsFileOfType(file, ".mb") || IsFileOfType(file, ".ma") || IsFileOfType(file, ".max") || IsFileOfType(file, ".blend") || IsFileOfType(file, ".obj") || IsFileOfType(file, ".3ds") || IsFileOfType(file, ".dxf");
        }
        ...
        {
            return IsFileOfType(file, ".wav") || IsFileOfType(file, ".mp3") || IsFileOfType(file, ".ogg") || IsFileOfType(file, ".aif") || IsFileOfType(file, ".xm") || IsFileOfType(file, ".mod") || IsFileOfType(file, ".it") || IsFileOfType(file, ".s3m");
        }
        ...
        public static bool IsAnimationFile(this string file)
        {
            return IsFileOfType(file, ".anim")
                ...
                ||                           // animator controller (mecanim state machine)
        ...
        public static bool IsUnityAssetFile(this string file)
        {
            return IsFileOfType(file, ".unity")
                ...
                || // scriptable objects, terrain files
        ...
        {
            return filepath.DoesFileBeginWith("BRT_") || filepath.DoesFileBeginWith("DldUtil_") || IsFileInAPath(filepath, "/BuildReport/");
        }
        ...
        {
            return IsFileInAPath(filepath, "/.svn/") || IsFileInAPath(filepath, "/.git/") || IsFileInAPath(filepath, "/.cvs/");
        }
        ...
        {
            return IsUselessFile(filepath) || (!IsFileInBuildReportFolder(filepath) && !IsFileInEditorFolder(filepath) && !IsFileInVersionControlMetadataFolder(filepath) && !IsFileAUnixHiddenFile(filepath));
        }
        ...
        {
            if (IsBuiltInAsset(assetPath)) return GetBuiltInAssetHeader(assetPath);

            var lastSlash = assetPath.LastIndexOf("/", StringComparison.Ordinal);
            if (lastSlash > -1) return assetPath.Substring(0, lastSlash + 1);
        ...
        {
            if (IsBuiltInAsset(assetPath)) return GetBuiltInAssetFilename(assetPath);
        ...
        private static string GetBuiltInAssetHeader(string assetPath)
        {
            var lastSlash = assetPath.LastIndexOf("/", StringComparison.Ordinal);
            ...
            if (colon > -1) return assetPath.Substring(0, colon + 1);

            return assetPath;
        }

        private static string GetBuiltInAssetFilename(string assetPath)
        {
            var hasSlash = assetPath.IndexOf("/", StringComparison.Ordinal) > 0;

            if (hasSlash) return assetPath.GetFileNameOnly();

            var idxOfColon = assetPath.IndexOf(":", StringComparison.Ordinal);
            if (idxOfColon > -1)
            {
        ...
        public static string GetPackageFileContents(string filename)
        {
            ...
            var defaultBuildReportToolFullPath =
                Application.dataPath + "/" + Options.BUILD_REPORT_TOOL_DEFAULT_FOLDER_NAME;

            var filePath = defaultBuildReportToolFullPath + "/" + filename;

            if (System.IO.File.Exists(filePath)) return GetTextFileContents(filePath);
            ...
            var folderPath = FindAssetFolder(Application.dataPath,
        ...
        public static bool ShowFileDeleteProgress(
            int    deletedSoFar,
            int    totalToDelete,
            string filepath,
        ...
        {
            var progress = (deletedSoFar + 1) / (float)totalToDelete;

            if (EditorUtility.DisplayCancelableProgressBar(
                string.Format("Deleting file {0} of {1} ({2} left)",
                    (deletedSoFar + 1).ToString(),
                    totalToDelete.ToString(),
                    (totalToDelete - deletedSoFar - 1).ToString()),
                filepath,
                progress))
            {
                EditorUtility.ClearProgressBar();

                var filesReallyDeletedPlural = deletedSoFar > 1 ? "s" : "";

                var    cancelTitle = "Delete operation canceled";
                string cancelMsg;

                if (deletedSoFar > 0)
                {
                    cancelMsg = string.Format("Only {0} file{1} (of {2}) deleted.",
                        deletedSoFar.ToString(),
                        filesReallyDeletedPlural,
                        totalToDelete.ToString());
        ...
        public static void OpenInFileBrowserTest()
        {
            ...
            var path = "/Users/Ferds/UnityBuildReports/test4.xml";
        ...
        public static bool IsInMacOS => SystemInfo.operatingSystem.IndexOf("Mac OS", StringComparison.Ordinal) != -1;

        public static bool IsInWinOS => SystemInfo.operatingSystem.IndexOf("Windows", StringComparison.Ordinal) != -1;

        public static string NameOfOSFileBrowser => IsInMacOS ? "Finder" : "Explorer";

        public static string NameOfOSTrashFolder => IsInMacOS ? "Trash folder" : "Recycle Bin";

        private static string GetEditorLogFileInWindows(string editorFilename = "Editor.log")
        {
            var editorLogSubPath = "/Unity/Editor/" + editorFilename;
            ...
            {
                var nonXpStyleAppDataPath = localAppDataVar.Replace("\\", "/");
                if (System.IO.Directory.Exists(nonXpStyleAppDataPath)) return nonXpStyleAppDataPath + editorLogSubPath;
            }

            // didn't find it in LOCALAPPDATA
            // try USERPROFILE (WinXP style)

            var userProfileVar = Environment.GetEnvironmentVariable("USERPROFILE");

            if (!string.IsNullOrEmpty(userProfileVar))
            {
                var xpStyleAppDataPath = userProfileVar.Replace("\\", "/") + "/Local Settings/Application Data";
                if (System.IO.Directory.Exists(xpStyleAppDataPath)) return xpStyleAppDataPath + editorLogSubPath;
            }
        ...
        public static string EditorLogDefaultPath
        {
            get
            {
                if (Environment.OSVersion.Platform == PlatformID.Unix || Environment.OSVersion.Platform == PlatformID.MacOSX) return UserHomePath + "/Library/Logs/Unity/Editor.log";
        ...
        public static string EditorPrevLogPath
        {
            get
            {
                if (Environment.OSVersion.Platform == PlatformID.Unix || Environment.OSVersion.Platform == PlatformID.MacOSX) return UserHomePath + "/Library/Logs/Unity/Editor-prev.log";
        ...
        public static string UsedEditorLogPath
        {
            get
            {
                if (IsDefaultEditorLogPathOverridden) return Options.EditorLogOverridePath;
        ...
        public static string EditorLogPathOverrideMessage
        {
            get
            {
                if (IsDefaultEditorLogPathOverridden) return "(Overridden)";

                return "(Default)";
            }
        }

        public static bool IsDefaultEditorLogPathOverridden => !string.IsNullOrEmpty(Options.EditorLogOverridePath);

        public static bool UsedEditorLogExists => System.IO.File.Exists(UsedEditorLogPath);

        public static string GetBuildManagedFolder(string buildFilePath)
        {
            var buildFolder = buildFilePath;
        ...
        private static string GetProjectTempStagingArea(string projectDataPath)
        {
            var          tempFolder = projectDataPath;
        ...
        {
            var tempFolder = GetProjectTempStagingArea(projectDataPath) + "/Data/Managed/";
        ...
        {
            var tempFolder = GetProjectTempStagingArea(projectDataPath) + "/assets/bin/Data/Managed/";
        ...
        public static bool AttemptGetUnityFolderMonoDLLs(
            bool                  wasWebBuild,
            bool                  wasAndroidApkBuild,
            string                editorAppContentsPath,
            ApiCompatibilityLevel monoLevel,
            StrippingLevel        codeStrippingLevel,
            out string            path,
            out string            higherPriorityPath
        )
        {
            var success = false;
            ...
            {
                tryPath = pathTries[n];
                if (System.IO.Directory.Exists(tryPath)) break;
        ...
        public static SizePart CreateSizePartFromFile(
            string filename,
            string fileFullPath,
        ...
        )
        {
            ...
            if (System.IO.File.Exists(fileFullPath))
            {
                if (getRawSize)
                {
                    var fileSizeBytes = GetFileSizeInBytes(fileFullPath);
        ...
        private static void SaveTextFile(string saveFilePath, string data)
        {
            var folder = System.IO.Path.GetDirectoryName(saveFilePath);
		...
		Debug.LogError("Current build target is set to Web Player. Cannot perform file input/output when in Web Player.");
            #else
            var
                write = new System.IO.StreamWriter(saveFilePath,
                    false,
        ...
        private static string FixXmlBuildReportFile(string serializedBuildInfoFilePath)
        {
            var xmlData = GetTextFileContents(serializedBuildInfoFilePath);
        ...
        {
            if (!System.IO.File.Exists(serializedBuildInfoFilePath)) return null;

            BuildInfo ret = null;

            var x = new System.Xml.Serialization.XmlSerializer(typeof(BuildInfo));

            var correctedXmlData = FixXmlBuildReportFile(serializedBuildInfoFilePath);
        ...
        private const string SAVE_DATE_TIME_FORMAT = "yyyyMMMdd-HHmmss";
        ...
        public static string GetAssetDependenciesDefaultFilename(
            string   projectName,
            string   buildType,
            DateTime timeGot
        )
        ...
        public static string GetTextureDataDefaultFilename(
            string   projectName,
            string   buildType,
            DateTime timeGot
        )
        ...
        public static string GetMeshDataDefaultFilename(
            string   projectName,
            string   buildType,
            DateTime timeGot
        )
        ...
        public static string GetUnityBuildReportDefaultFilename(
            string   projectName,
            string   buildType,
            DateTime timeGot
        )
        ...
        public static string SerializeAtFolder<T>(
            T      data,
        ...
        ) where T : class, IDataFile
        {
            ...
            {
                if (!System.IO.Directory.Exists(folderPathToSaveTo)) System.IO.Directory.CreateDirectory(folderPathToSaveTo);

                var lastChar = folderPathToSaveTo[folderPathToSaveTo.Length - 1];
