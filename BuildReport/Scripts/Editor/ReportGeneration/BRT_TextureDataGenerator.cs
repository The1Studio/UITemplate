...
namespace BuildReportTool
{
    public static class TextureDataGenerator
    {
        public static void CreateForUsedAssetsOnly(TextureData data, BuildInfo buildInfo, bool debugLog = false)
        {
            ...
            var buildPlatform = ReportGenerator.GetBuildPlatformFromString(buildInfo.BuildType, buildInfo.BuildTargetUsed);
        ...
        private static void AppendTextureData(TextureData data, BuildPlatform buildPlatform, IList<SizePart> assets, bool overwriteExistingEntries, bool debugLog = false)
        {
        ...
        private const int    ANISO_LEVEL_IF_VALUE_IS_NEGATIVE_ONE         = 1;
        private const int    COMPRESSION_QUALITY_IF_VALUE_IS_NEGATIVE_ONE = 50;
        private const string WRAP_MODE_IF_VALUE_IS_NEGATIVE_ONE           = "Repeat";

        private static TextureData.Entry CreateEntry(string assetPath, string platform, bool debugLog = false)
        {
            ...
            if (result.CompressionQuality == -1) result.CompressionQuality = COMPRESSION_QUALITY_IF_VALUE_IS_NEGATIVE_ONE;
            ...
            if (overrideSettings != null && overrideSettings.overridden)
            {
                ...
                if (result.OverridingCompressionQuality == -1) result.OverridingCompressionQuality = COMPRESSION_QUALITY_IF_VALUE_IS_NEGATIVE_ONE;
                ...
                if (debugLog && result.TextureFormat != result.OverridingTextureFormat)
                    Debug.LogFormat("TextureDataGenerator: {0} for {1} format overriden from {2} to {3}",
                        ...
                        result.OverridingTextureFormat);
                ...
                if (debugLog && result.MaxTextureSize != result.OverridingMaxTextureSize)
                    Debug.LogFormat("TextureDataGenerator: {0} for {1} max size overriden from {2} to {3}",
                        platform,
                        assetPath,
                        result.MaxTextureSize.ToString(),
                        result.OverridingMaxTextureSize.ToString());
            }
            else
                #endif
            {
            ...
            if (loadedTexture != null)
            {
                ...
                if (debugLog)
                    Debug.LogFormat("Got imported dimensions (using AssetDatabase.LoadAssetAtPath Texture) for {0} {1}x{2}",
                        assetPath,
                        result.ImportedWidth.ToString(),
                        result.ImportedHeight.ToString());
            }
            else
            {
                if (debugLog) Debug.LogErrorFormat("Could not get imported width and height, AssetDatabase.LoadAssetAtPath returned null for {0}", assetPath);
        ...
        private static string TextureTypeToReadableString(TextureImporterType textureType)
        {
            switch (textureType)
            {
                #if UNITY_5_5_OR_NEWER
				...
				case TextureImporterType.Bump:
					return "Normal Map";
                #endif
                case TextureImporterType.GUI:      return "GUI";
                case TextureImporterType.Sprite:   return "Sprite";
                ...
                case TextureImporterType.DirectionalLightmap: return "Directional Lightmap";
                #endif
                default: return textureType.ToString();
            }
        }

        private static string WrapModeToReadableString(TextureWrapMode wrapMode)
        {
            switch (wrapMode)
            {
                ...
                case TextureWrapMode.MirrorOnce: return "MirrorOnce";
                #endif
                default:
                    if ((int)wrapMode == -1) return WRAP_MODE_IF_VALUE_IS_NEGATIVE_ONE;
                    return string.Format("Unrecognized ({0})", wrapMode.ToString());
            }
        }

        #if UNITY_5_5_OR_NEWER
        private static string CompressionTypeToReadableString(TextureImporterCompression compression)
        {
            switch (compression)
            {
                case TextureImporterCompression.Uncompressed: return "Uncompressed";
                case TextureImporterCompression.Compressed:   return "Standard Compression";
                case TextureImporterCompression.CompressedHQ: return "High Quality, Low Compression";
                case TextureImporterCompression.CompressedLQ: return "Low Quality, High Compression";
        ...
        public static string GetPlatformString(BuildPlatform buildPlatform)
        {
            ...
            switch (buildPlatform)
            {
                ...
                case BuildPlatform.iOS:     return "iPhone";
                case BuildPlatform.Tizen:   return "Tizen";
                ...
                case BuildPlatform.Windows64:
                    return "Standalone";
                case BuildPlatform.WindowsStoreApp: return "Windows Store Apps";
                case BuildPlatform.XBOXOne:         return "XboxOne";
                case BuildPlatform.PS4:             return "PS4";
                case BuildPlatform.PSVitaNative:    return "PSP2";
                case BuildPlatform.WiiU:            return "WiiU";
                case BuildPlatform.Nintendo3DS:     return "Nintendo 3DS";
                case BuildPlatform.Switch:          return "Nintendo Switch";
                default:                            return null;
            }
        }
        ...
        /// </summary>
        private static readonly System.Reflection.MethodInfo TextureImporterGetSourceTextureInformation =
        ...
        /// </summary>
        private static readonly System.Reflection.MethodInfo TextureImporterGetWidthAndHeight =
            typeof(TextureImporter).GetMethod("GetWidthAndHeight",
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);

        private static object[] _getWidthAndHeightParameters;
        #endif

        private static ImageUtility.Dimensions GetImageRealWidthAndHeight(string assetPath, TextureImporter textureImporter, bool debugLog = false)
        {
            ...
            if (TextureImporterGetSourceTextureInformation != null)
            {
                ...
                if (debugLog)
                    Debug.LogFormat("Got dimensions (using GetSourceTextureInformation) for {0} {1}x{2}",
                        assetPath,
                        sourceTextureInfo.width.ToString(),
                        sourceTextureInfo.height.ToString());
            ...
            {
                if (_getWidthAndHeightParameters == null) _getWidthAndHeightParameters = new object[] { new int(), new int() };
                ...
                if (debugLog)
                    Debug.LogFormat("Got dimensions (using GetWidthAndHeight) for {0} {1}x{2}",
                        assetPath,
                        returnValue.Width.ToString(),
                        returnValue.Height.ToString());

                return returnValue;
            }
            #endif
            if (assetPath.EndsWith(".jpg", System.StringComparison.OrdinalIgnoreCase) || assetPath.EndsWith(".jpeg", System.StringComparison.OrdinalIgnoreCase) || assetPath.EndsWith(".png", System.StringComparison.OrdinalIgnoreCase) || assetPath.EndsWith(".bmp", System.StringComparison.OrdinalIgnoreCase) || assetPath.EndsWith(".gif", System.StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (debugLog)
                    Debug.LogFormat("Got dimensions (using ImageDimensions.Get.AsTuple) for {0} {1}x{2}",
                        assetPath,
                        returnValue.Width.ToString(),
                        returnValue.Height.ToString());
