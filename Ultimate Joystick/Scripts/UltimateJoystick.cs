...
#endif
{
    ...
    public void OnPointerDown(PointerEventData touchInfo)
    {
        ...
        if (boundary == Boundary.Circular && !customActivationRange)
        {
            ...
            float distance = Vector2.Distance(
                (Vector2)(ParentCanvas.transform.InverseTransformPoint(joystickBase.position) * ParentCanvas.scaleFactor) + ((canvasRectTrans.sizeDelta * ParentCanvas.scaleFactor) / 2),
                touchInfo.position);
        ...
        if (dynamicPositioning || disableVisuals)
        {
            ...
            joystickBase.localPosition = (Vector2)baseTrans.InverseTransformPoint(ParentCanvas.transform.TransformPoint(touchInfo.position / ParentCanvas.scaleFactor)) -
                                         (canvasRectTrans.sizeDelta / 2);
    ...
    /// </summary>
    void OnTransformParentChanged() { UpdateParentCanvas(); }
    ...
    void UpdateJoystickPositioning()
    {
        ...
        Vector2 joystickPosition = new Vector2(canvasRectTrans.sizeDelta.x * (positionHorizontal / 100) - (textureSize * (positionHorizontal / 100)) + (textureSize / 2),
            canvasRectTrans.sizeDelta.y * (positionVertical / 100) - (textureSize * (positionVertical / 100)) + (textureSize / 2)) - (canvasRectTrans.sizeDelta / 2);
        ...
        if (customActivationRange)
        {
            ...
            baseTrans.localPosition =
                new Vector2(canvasRectTrans.sizeDelta.x * (activationPositionHorizontal / 100) - (baseTrans.sizeDelta.x * (activationPositionHorizontal / 100)) + (baseTrans.sizeDelta.x / 2),
                    canvasRectTrans.sizeDelta.y * (activationPositionVertical / 100) - (baseTrans.sizeDelta.y * (activationPositionVertical / 100)) + (baseTrans.sizeDelta.y / 2)) -
                (canvasRectTrans.sizeDelta / 2);
        ...
        joystickRect = new Rect(new Vector2(baseLocalPosition.x - (baseSizeDelta.x / 2), baseLocalPosition.y - (baseSizeDelta.y / 2)) + ((canvasRectTrans.sizeDelta * ParentCanvas.scaleFactor) / 2),
            baseSizeDelta);
    }
        ...
        if (tensionType == TensionType.Directional)
        {
            ...
            else
            {
                ...
                if (TensionAccents[1] != null)
                    TensionAccents[1].color = Color.Lerp(tensionColorNone, tensionColorFull,
                        Mathf.Abs(joystickAxis.x) <= tensionDeadZone ? 0 : (Mathf.Abs(joystickAxis.x) - tensionDeadZone) / (1.0f - tensionDeadZone));
            ...
            else
            {
                ...
                if (TensionAccents[2] != null)
                    TensionAccents[2].color = Color.Lerp(tensionColorNone, tensionColorFull,
                        Mathf.Abs(joystickAxis.y) <= tensionDeadZone ? 0 : (Mathf.Abs(joystickAxis.y) - tensionDeadZone) / (1.0f - tensionDeadZone));
    ...
    /// </summary>
    public float GetDistance() { return Vector3.Distance(joystick.localPosition * ParentCanvas.scaleFactor, Vector3.zero) / radius; }
    ...
    /// </summary>
    public bool GetJoystickState() { return joystickState; }

    /// <summary>
    /// Returns the tap count to the Ultimate Joystick.
    /// </summary>
    public bool GetTapCount() { return tapCountAchieved; }
