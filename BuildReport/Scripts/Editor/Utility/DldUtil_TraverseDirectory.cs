...
namespace DldUtil
{
    public static class TraverseDirectory
    {
        ...
        private const long OUTER_LOOP_LIMIT = 1000000;    // 1 million
        private const long FILE_LIMIT       = 4000000000; // 4 billion

        public static IEnumerable<string> Do(string path)
        {
            var traversal = new Stack<TraversalStack>(5);
            ...
            while (traversal.Count > 0)
            {
                ...
                if (infiniteCounterStack > OUTER_LOOP_LIMIT) break;

                currentStack = traversal.Peek();
                //Debug.Log("in " + currentStack.TopFolder);

                var toGoDeeper = false;

                var allInCurrentFolder = Directory.GetFileSystemEntries(currentStack.TopFolder);

                infiniteCounterFile = 0;
                for (long n = currentStack.ChildIdxToGoOnReturn, len = allInCurrentFolder.Length; n < len; ++n)
                {
                    ++infiniteCounterFile;
                    if (infiniteCounterFile > FILE_LIMIT) break;
