...
namespace BuildReportTool
{
    public static class MeshDataGenerator
    {
        ...
        private static void AppendMeshData(MeshData data, IList<SizePart> assets, bool overwriteExistingEntries, bool debugLog = false)
        {
        ...
        private static readonly List<SkinnedMeshRenderer> SkinnedMeshRenderers = new();
        #if UNITY_5_5_OR_NEWER
        private static readonly List<int> TriangleBuffer = new();
        #endif

        private static MeshData.Entry CreateEntry(string assetPath, bool debugLog = false)
        {
            ...
            if (loadedModel != null)
            {
                ...
                for (var n = 0; n < MeshFilters.Count; ++n)
                {
                    var subMeshCount = MeshFilters[n].sharedMesh.subMeshCount;
                    totalSubMeshCount += subMeshCount;
                    totalVertexCount  += MeshFilters[n].sharedMesh.vertexCount;

                    for (var m = 0; m < subMeshCount; ++m)
                    {
                ...
                for (var n = 0; n < SkinnedMeshRenderers.Count; ++n)
                {
                    var subMeshCount = SkinnedMeshRenderers[n].sharedMesh.subMeshCount;
                    totalSubMeshCount += subMeshCount;
