    ...
    public static class TreeElementUtility
    {
        ...
        public static T ListToTree<T>(IList<T> list) where T : TreeElement
        {
            ...
            for (var parentIndex = 0; parentIndex < list.Count; parentIndex++)
            {
                ...
                if (childCount != 0)
                {
                    childList  = new(childCount); // Allocate once
                    childCount = 0;
                    for (var i = parentIndex + 1; i < list.Count; i++)
                    {
        ...
        public static void ValidateDepthValues<T>(IList<T> list) where T : TreeElement
        {
            ...
            for (var i = 0; i < list.Count - 1; i++)
            {
                ...
                if (nextDepth > depth && nextDepth - depth > 1) throw new ArgumentException(string.Format("Invalid depth info in input list. Depth cannot increase more than 1 per row. Index {0} has depth {1} while index {2} has depth {3}", i, depth, i + 1, nextDepth));
            }
